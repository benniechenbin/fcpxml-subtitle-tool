...
async function processFile(file) {
  ...
  try {
    ...
    else if (ext === "fcpxml" || ext === "xml")
      globalBlocks = parseFCPXML(content);
  ...
  try {
    ...
    if (data.status === "success" && data.srt_text) {
      ...
      if (globalBlocks.length > 0) {
        globalBlocks.sort((a, b) => a.start - b.start);
        log(
          `已成功提取 ${globalBlocks.length} 条字幕！现在可以继续使用 AI 翻译或导出了。`,
          "SUCCESS",
        );
      } else {
...
const PROVIDER_MODELS = {
  ...
  webllm: [
    // WebLLM 必须精确匹配 MLC 库支持的模型 ID
    {
      ...
      label: "Qwen 2.5 (1.5B) - Fast & Good",
    },
    {
      value: "Llama-3-8B-Instruct-q4f32_1-MLC",
      label: "Llama 3 (8B) - Standard",
    },
    { value: "Phi-3-mini-4k-instruct-q4f16_1-MLC", label: "Phi-3 Mini (3.8B)" },
  ],
  custom: [{ value: "default", label: "Default / Auto-Detect" }],
};
...
window.resetConnectionState = function () {
  ...
  btn.className =
    "w-8 h-8 flex items-center justify-center bg-gray-800 border border-gray-700 rounded hover:bg-gray-700 transition-all text-gray-400";
...
window.testConnection = async function () {
  ...
  try {
    ...
    if (res.ok) {
      ...
      btn.className =
        "w-8 h-8 flex items-center justify-center bg-gray-800 border border-green-500/50 rounded text-green-400 shadow-[0_0_10px_rgba(74,222,128,0.2)]";
      statusText.innerText = "Connected";
      statusText.className =
        "text-[9px] text-green-500 font-mono transition-colors";
    ...
    btn.className =
      "w-8 h-8 flex items-center justify-center bg-gray-800 border border-red-500/50 rounded text-red-400";
    statusText.innerText = "Error";
    statusText.className =
      "text-[9px] text-red-400 font-mono transition-colors";
...
window.processWithAI = async function () {
  ...
  switch (provider) {
    ...
    case "google":
      return log(
        "Google 原生 API 需要特殊代理，请使用 Custom 模式配合 OneAPI。",
        "WARN",
      );
    case "ollama":
  ...
  try {
    for (let i = 0; i < globalBlocks.length; i += BATCH_SIZE) {
      ...
      const requestBody = {
        ...
        temperature: 0.1,
        response_format:
          selectedModel.includes("reasoner") || provider === "ollama"
            ? undefined
            : { type: "json_object" },
      };

      log(
        `正在处理第 ${i + 1} - ${Math.min(i + BATCH_SIZE, globalBlocks.length)} 行...`,
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `API Error ${response.status}: ${errText.slice(0, 100)}...`,
        );
      }
      ...
      try {
        ...
        log(
          `⚠️ 第 ${Math.floor(i / BATCH_SIZE) + 1} 批次格式解析失败，跳过。`,
          "ERR",
        );
        continue;
      }
...
function generateXML(blocks, p) {
  ...
  if (fps === 25) frameDuration = "100/2500s";
    ...
    frameDuration = "1001/24000s"; // NTSC
  else if (fps === 29.97)
    frameDuration = "1001/30000s"; // NTSC
  ...
  blocks.forEach((block, index) => {
    ...
    if (isBilingualMode && block.text.includes("\n")) {
      ...
      const cnText = lines[0];
      const enText = lines.slice(1).join(" ");
...
function parseFCPXML(xmlStr) {
  ...
  titles.forEach((t) => {
    ...
    const offset = parseFCPTime(t.getAttribute("offset"));
    const duration = parseFCPTime(t.getAttribute("duration"));
    blocks.push({
      start: offset,
      end: offset + duration,
      text: rawText.trim(),
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.body.addEventListener("change", function (e) {
    ...
    if (e.target.tagName === "SELECT")
      msg = `Option changed: [${id}] -> ${val}`;
    else if (e.target.type === "color") msg = `Color update: [${id}] -> ${val}`;
    else if (e.target.type === "checkbox")
      msg = `Checkbox [${id}] -> ${e.target.checked ? "ON" : "OFF"}`;
